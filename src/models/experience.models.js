...
const experienceModel = {
    ...
    insertExperience: (data) => {
        const {id, position, company_name, started, ended, company_photo, description} = data
        return new Promise ((resolve, reject) =>
            db.query(`INSERT INTO experience (id, position, company_name, started, ended, company_photo, description) VALUES ('${id}', '${position}', '${company_name}', '${started}', '${ended}', '${company_photo}', '${description}')`, (error,result) => {
                if(!error){
                    resolve(result)
                }else{
                    reject(error)
    ...
    updateExperience: (data) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            started='${started}', ended='${ended}', company_photo=${company_photo}, description='${description}'
            WHERE id='${id}'`,
                (err, res) => {
                    if (err) {
                        reject(err);
