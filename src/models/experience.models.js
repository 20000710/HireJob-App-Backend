...
const experienceModel = {
    ...
    insertExperience: (data) => {
        const {id, position, company_name, started, ended, description, user_id} = data
        return new Promise ((resolve, reject) =>
            db.query(`INSERT INTO experience (id, position, company_name, started, ended, description, user_id) VALUES ('${id}', '${position}', '${company_name}', '${started}', '${ended}', '${description}', '${user_id}')`, (error,result) => {
                if(!error){
                    resolve(result)
                }else{
                    reject(error)
    ...
    updateExperience: (data) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            started='${started}', ended='${ended}', description='${description}', user_id='${user_id}' 
            WHERE id='${id}'`,
                (err, res) => {
                    if (err) {
                        reject(err);
